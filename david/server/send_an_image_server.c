...
snprintf(cmd, sizeof(cmd), "./convert %s %s %s", inputPath, outputPath, operation);
...
pthread_attr_destroy(&attr);
    ...
    // Main thread acting as producer
    while (1) {
        printf("Master thread: Accepting new connection...\n");
       // displayQueue(fifo); // Display queue after accepting a connection
